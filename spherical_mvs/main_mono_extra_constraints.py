...
from network_resnet_extra_constraints import ResNet360
from util import *
import weight_init
...
parser.add_argument('--model', default='psmnet',
                    help='select model')
parser.add_argument('--input_dir', default='../data/Realistic',
...
parser.add_argument('--visualize_path', default='./visualize_monocular_extra_constraints',
                    help='save checkpoint path')                    
...
first_network = ResNet360(out_channels=3, norm_type='batchnorm', activation='relu', aspp=False, output_boundary=True)
second_network = ResNet360(in_channels=7, norm_type='batchnorm', activation='relu', aspp=True)
weight_init.initialize_weights(first_network, init="xavier", pred_bias=float(5.0))
weight_init.initialize_weights(second_network, init="xavier", pred_bias=float(5.0))
first_network = convert_model(first_network)
second_network = convert_model(second_network)

# parallel on multi gpu
first_network = nn.DataParallel(first_network)
second_network = nn.DataParallel(second_network)
...
optimizer = optim.Adam(list(first_network.parameters()) + list(second_network.parameters()), 
...
# Train Function -------------------
def train(rgb, depth, normal, edge, mask, batch_idx):
    #mask = mask>0
    ...
    optimizer.zero_grad()
    # Loss -------------------------------------------- 
    outputs_normal, outputs_boundary = first_network(rgb)
    input_second = torch.cat([rgb, outputs_normal, outputs_boundary], 1)
    outputs = torch.abs(second_network(input_second))
    ...
    plane_loss = L.direct.calculate_berhu_loss(plane_pred.view(-1, 1, 256, 512), plane.view(-1, 1, 256, 512),                
               mask=mask, weights=attention_weights)
    boundary_loss = L.direct.calculate_berhu_loss(outputs_boundary.view(-1, 1, 256, 512), edge.view(-1, 1, 256, 512),                
               mask=mask, weights=attention_weights)
    #print(normal_loss.item(), plane_loss.item(), depth_loss.item(), boundary_loss.item())
    loss = normal_loss + plane_loss + depth_loss + 0.1*depth_smoothness_loss + boundary_loss   
    #--------------------------------------------------
    ...
    edge_pred = outputs_boundary.detach().cpu().numpy()
    if batch_idx % visualize_interval == 0 and batch_idx > 0:
            ...
            edge_img = edge[0, :, :]
            edge_pred_img = edge_pred[0, 0, :, :]
            edge_img = cv2.normalize(edge_img,None,255.0,norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC1)
            edge_pred_img = cv2.normalize(edge_pred_img,None,255.0,norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8UC1)
            ...
            cv2.imwrite('{}/edge_{}.png'.format(result_view_dir, batch_idx), 255 - edge_img)
            cv2.imwrite('{}/edge_pred_{}.png'.format(result_view_dir, batch_idx), 255 - edge_pred_img)
...
# Valid Function -----------------------
def val(rgb, depth, normal, edge, mask, batch_idx):
    ...
    if batch_idx % 100 == 0:
            ...
            cv2.imwrite('{}/test_edge_{}.png'.format(result_view_dir, batch_idx), 255-edge_img)
            cv2.imwrite('{}/test_edge_pred_{}.png'.format(result_view_dir, batch_idx), 255-edge_pred_img)
...
def main():
    ...
    for epoch in range(start_epoch+1, args.epochs+1):
        ...
        for batch_idx, (rgb, depth, depth_mask) in tqdm(enumerate(train_dataloader),desc='it'):
            ...
            loss = train(rgb, depth, normal, edge, depth_mask, batch_idx)
        ...
        for batch_idx, (rgb, depth, depth_mask) in tqdm(enumerate(val_dataloader),desc='it'):
            ...
            rgb, depth, depth_mask = rgb.cuda(), depth.cuda(), depth_mask.cuda()
            normal, edge = depth2normal_gpu(depth)
            val_output = val(rgb, depth, normal, edge, depth_mask, batch_idx)
