...
parser.add_argument('--visualize_path', default='./visualize_1view',
                    help='save checkpoint path')                    
...
scheduler1 = optim.lr_scheduler.MultiStepLR(optimizer1, [30, 60], 0.5)
scheduler2 = optim.lr_scheduler.MultiStepLR(optimizer2, [30, 60], 0.5)
#---------------------
        ...
        for batch_idx, (rgb, depth, depth_mask) in tqdm(enumerate(train_dataloader)):
            ...
            for bl, direction in zip(baseline, baseline_direction):
                if direction == 'vertical':
                    render.append(dibr_vertical(coarse_depth_pred, rgb, uvgrid, sgrid, baseline=bl))
                elif direction == 'horizontal':
                    render.append(dibr_horizontal(coarse_depth_pred, rgb, uvgrid, sgrid, baseline=bl))
                else:
                    raise NotImplementedError

            stereo_loss = train(rgb, render, depth.squeeze(1), depth_mask.squeeze(1), batch_idx)
            
            #need to balance the first network loss and second stereo matching network loss
            alpha, beta = 0.02, 1
            if epoch == 1 and batch_idx<args.start_learn:
            #if epoch < 3:
                ...
                loss = stereo_loss*alpha + coarse_loss*beta
            loss.backward()
            optimizer1.step()
            optimizer2.step()
