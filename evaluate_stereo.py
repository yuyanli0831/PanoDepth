...
def val(target, render, depth, mask, batch_idx):
    ...
    if batch_idx % 20 == 0:
        ...
        depth_down_img = depth[0, :, :]
        depth_pred_img = depth_prediction[0, :, :]
        error_map = np.abs(depth_down_img - depth_pred_img) * (depth_down_img > 0)
        cv2.imwrite(result_view_dir + '/test_gt_' + str(batch_idx) + '.png', gt_img*255)
        render_img = render_np[0, :, :, :].transpose(1,2,0)
        cv2.imwrite(result_view_dir + '/test_render' +  str(batch_idx) + '.png', render_img*255)
        ...
        plot.imsave(result_view_dir + '/test_depth_pred_' + str(batch_idx) + '.png', depth_pred_img, cmap="jet")
        plot.imsave(result_view_dir + '/test_depth_gt_' + str(batch_idx) + '.png', depth_down_img, cmap="jet")
        plot.imsave(result_view_dir + '/test_error_' + str(batch_idx) + '.png', error_map, cmap="jet")
    return output3
...
def main():
    stereo_network.eval()
    for batch_idx, (up, down, depth_up, depth_down, depth_upmask, depth_downmask) in enumerate(tqdm(val_dataloader)):
            sgrid = S360.grid.create_spherical_grid(512).cuda()
