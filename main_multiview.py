...
scheduler2 = optim.lr_scheduler.MultiStepLR(optimizer2, [20, 40], 0.33)
#---------------------
...
def main():
    ...
    for epoch in tqdm(range(start_epoch+1, args.epochs+1), desc='Epoch'):
        ...
        for batch_idx, (rgb, depth, depth_mask) in tqdm(enumerate(train_dataloader)):
            ...
            output, stereo_loss = train(rgb, render_list, depth.squeeze(1), depth_mask.squeeze(1), batch_idx)
            loss = stereo_loss
