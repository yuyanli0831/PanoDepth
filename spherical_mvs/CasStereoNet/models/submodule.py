...
#from network_syn_mapped import SphereAvgPool2D

Align_Corners = False
Align_Corners_Range = False
#avgpool = SphereAvgPool2D(stride=1)
...
def get_cur_depth_range_samples(cur_depth, 
    ...
    if not using_ns:
        ...
        cur_depth_max_inv = (1/cur_depth - ndepth/2*depth_inteval_pixel).clamp(min=1/max_depth, max=1/min_depth)
        cur_depth_min_inv = (1/cur_depth + ndepth/2*depth_inteval_pixel).clamp(min=1/max_depth, max=1/min_depth)
        new_inteval = (cur_depth_min_inv - cur_depth_max_inv)/(ndepth-1) 
        cur_depth_min = 1/cur_depth_min_inv
        cur_depth_max = 1/cur_depth_max_inv
        interval = torch.arange(0, ndepth, device=cur_depth.device,
                                    dtype=cur_depth.dtype,
                                    requires_grad=False).reshape(1, -1, 1, 1) * new_inteval.unsqueeze(1)
        
        depth_range_samples = 1/(1/cur_depth_max.unsqueeze(1) + \
        (1/cur_depth_min.unsqueeze(1)-1/cur_depth_max.unsqueeze(1))*interval/(ndepth-1))

    else:
        cur_depth_max_ns = F.max_pool2d(cur_depth, kernel_size=ns_size, padding=ns_size//2, stride=1)
        cur_depth_min_ns = torch.abs(F.max_pool2d(-cur_depth, kernel_size=ns_size, padding=ns_size//2, stride=1))
        cur_depth_max_inv = (1/cur_depth_max_ns - ndepth/2*depth_inteval_pixel).clamp(min=1/max_depth, max=1/min_depth)
        cur_depth_min_inv = (1/cur_depth_min_ns + ndepth/2*depth_inteval_pixel).clamp(min=1/max_depth, max=1/min_depth)
        new_inteval = (cur_depth_min_inv - cur_depth_max_inv)/(ndepth-1) 
        cur_depth_min = 1/cur_depth_min_inv
        cur_depth_max = 1/cur_depth_max_inv
        interval = torch.arange(0, ndepth, device=cur_depth.device,
        ...
        depth_range_samples = 1/(1/cur_depth_max.unsqueeze(1) + \
        (1/cur_depth_min.unsqueeze(1)-1/cur_depth_max.unsqueeze(1))*interval/(ndepth-1))
    return depth_range_samples, interval, cur_depth_min, cur_depth_max
...
def get_depth_range_samples(cur_depth, 
    ...
    if cur_depth is None:
        cur_depth = torch.ones(shape, device=device, dtype=dtype)
        cur_depth_min = min_depth*cur_depth
        cur_depth_max = max_depth*cur_depth
 
        interval = torch.arange(0, ndepth, device=cur_depth.device,
                    dtype=cur_depth.dtype,
                    requires_grad=False).reshape(1, -1, 1, 1).repeat(shape[0], 1, shape[1], shape[2])
        
        depth_range_sample = 1/(1/cur_depth_max.unsqueeze(1) + \
        (1/cur_depth_min.unsqueeze(1)-1/cur_depth_max.unsqueeze(1))*interval/(ndepth-1))
        #print(depth_range_sample.shape)

    else:
        depth_range_sample, interval, cur_depth_min, cur_depth_max = get_cur_depth_range_samples(cur_depth, ndepth, depth_inteval_pixel, shape, ns_size, using_ns)

    return depth_range_sample, interval, cur_depth_min, cur_depth_max
