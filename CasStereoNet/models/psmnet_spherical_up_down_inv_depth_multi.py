...
class GetCostVolume(nn.Module):
    ...
    def forward(self, ref, target, depth_range_samples, ndisp, baseline, direction):
        ...
        #volume_variance = torch.cat([volume_variance, xx_channel, yy_channel, zz_channel], dim=1)
...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right, baseline, direction, min_depth=0.3, max_depth=8.0):
        refimg_msfea = []
        num_ref = len(left)
        for ref in left:
