...
class ResNet360(nn.Module):
    ...
    def __init__(
        ...
        self.prediction = create_conv(wf, out_channels, conv_type, \
            kernel_size=7, padding=3, width=width)
        
        self.prediction2 = None
        if output_boundary:
            self.prediction2 = create_conv(wf, 1, conv_type, \
                kernel_size=3, padding=1, width=width)   
    ...
    def forward(self, input):
        ...
        if self.prediction2 is not None:
            prediction2 = self.prediction2(output)
