...
class GetCostVolume(nn.Module):
    ...
    def forward(self, x, y, depth_range_samples, ndisp):
        ...
        disp_range_samples = 0.1*400/(depth_range_samples+1e-3)
        #print(depth_range_samples.max())
        #print(depth_range_samples.min())
        #print(disp_range_samples.max())
        #print(disp_range_samples.min())
        #exit()
...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right, min_depth=0.5, max_depth=8.0):
        left = left[:,:3,:,:]
        right = right[:,:3,:,:]
