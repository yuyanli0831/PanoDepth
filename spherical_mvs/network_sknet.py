...
class SKNet360(nn.Module):
    ...
    def __init__(
        ...
        """
        assert(depth >= 0)
        super(SKNet360, self).__init__()
        model = (
            [
                create_conv(in_channels, wf, 'coord', \
        ...
        for i in range(n_downsampling): 
            ...
            model += (
                [
                    create_conv(wf * mult, wf * mult * 2, 'coord', \
            ...
            for i in range(2):
                model += [ResnetBlock(wf * mult * 2, activation=activation, \
                    norm_type=norm_type, conv_type='skconv', \
        ...
        model += [create_conv(wf, out_channels, 'coord', \
...
class SKConv(nn.Module):
    def __init__(self, features, M=2, G=16, r=16, stride=1 ,L=32):
        """ Constructor
        Args:
...
def create_conv(in_size, out_size, conv_type, padding=1, stride=1, kernel_size=3, width=512):
    ...
    elif conv_type == 'skconv':
        return SKConv(in_size, stride=stride)
