...
class CostAggregation(nn.Module):
    ...
    def forward(self, cost, FineD, FineH, FineW, depth_range_samples, interval, min_depth=0.5, max_depth=8.0):
...
class GetCostVolume(nn.Module):
    ...
    def forward(self, x, y, depth_range_samples, ndisp, baseline):
        assert (x.is_contiguous() == True)

        bs, channels, height, width = x.size()
        cost = x.new().resize_(bs, channels * 2, ndisp, height, width).zero_()  #(B, 2C, D, H, W)
        
        disp_range_samples = baseline*400/(depth_range_samples+1e-3)
        ...
        cur_disp_coords_y[torch.isnan(cur_disp_coords_y)] = 0
        cur_disp_coords_y[torch.isinf(cur_disp_coords_y)] = 0
...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right, baseline, min_depth=0.5, max_depth=8.0):
        left = left[:,:3,:,:]
        right = right[:,:3,:,:]
        ...
        for stage_idx in range(self.num_stage):
            ...
            cost = self.get_cv(refimg_fea, targetimg_fea,
                               ...
                               ndisp=self.ndisps[stage_idx]//int(stage_scale), baseline=baseline)
