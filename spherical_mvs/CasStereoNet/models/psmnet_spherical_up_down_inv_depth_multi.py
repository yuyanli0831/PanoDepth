...
class feature_extraction(nn.Module):
    ...
    def forward(self, x):
        ...
        output_branch1 = F.interpolate(output_branch1, (output_skip.size()[2],output_skip.size()[3]),mode='bilinear', align_corners=Align_Corners)

        output_branch2 = self.branch2(output_skip)
        output_branch2 = F.interpolate(output_branch2, (output_skip.size()[2],output_skip.size()[3]),mode='bilinear', align_corners=Align_Corners)

        output_branch3 = self.branch3(output_skip)
        output_branch3 = F.interpolate(output_branch3, (output_skip.size()[2],output_skip.size()[3]),mode='bilinear', align_corners=Align_Corners)

        output_branch4 = self.branch4(output_skip)
        output_branch4 = F.interpolate(output_branch4, (output_skip.size()[2],output_skip.size()[3]),mode='bilinear', align_corners=Align_Corners)
...
class GetCostVolume(nn.Module):
    ...
    def forward(self, ref, target, depth_range_samples, ndisp, baseline, direction):
        assert (target.is_contiguous() == True)

        bs, channels, height, width = target.size()
        num_views = len(ref) + 1
        cost = target.unsqueeze(2).repeat(1, 1, ndisp, 1, 1)
        ...
        volume_sum = cost
        volume_sq_sum = cost ** 2
        del cost
        for ref_fea, bl, direc in zip(ref, baseline, direction):
            if direc == 'vertical':
                disp = torch.stack(
                    (
                        torch.zeros_like(depth_range_samples),
                        S360.derivatives.dtheta_vertical(sgrid, depth_range_samples, bl)
                    ),
                     dim=-1
                    )  #(B, D, H, W, 2)
            ...
            if direc == 'horizontal':
                disp = torch.stack(
                    (
                        S360.derivatives.dphi_horizontal(sgrid, depth_range_samples, bl),
                        S360.derivatives.dtheta_horizontal(sgrid, depth_range_samples, bl)
                    ),
                     dim=-1
                    )  #(B, D, H, W, 2)
            ...
            cur_disp_coords_y = render_coords[:,:,:,:,1]
            cur_disp_coords_x = render_coords[:,:,:,:,0]

            coords_x = cur_disp_coords_x / ((width - 1.0) / 2.0) - 1.0  # trans to -1 - 1
            coords_y = cur_disp_coords_y / ((height - 1.0) / 2.0) - 1.0
            grid = torch.stack([coords_x, coords_y], dim=4)   #(B, D, H, W, 2)
            cost_cur = F.grid_sample(ref_fea, grid.view(bs, ndisp * height, width, 2), 
                                     mode='bilinear',
                                     padding_mode='zeros').view(bs, channels, ndisp, height, width)
            if self.training:
                volume_sum = volume_sum + cost_cur
                volume_sq_sum = volume_sq_sum + cost_cur ** 2
            else:
                volume_sum += cost_cur
                volume_sq_sum += cost_cur.pow_(2)
...
class PSMNet(nn.Module):
    def __init__(self, ndisps, disp_interval_pixel, using_ns, ns_size, grad_method="detach",
        ...
        cr_feats_in_chs = [chs for chs in self.feature_extraction.out_channels]
    ...
    def forward(self, left, right, baseline, direction, min_depth=0.1, max_depth=8.0):
        refimg_msfea = []
        num_ref = len(left)
        for ref in left:
            refimg_msfea.append(self.feature_extraction(ref))
        ...
        for stage_idx in range(self.num_stage):
            ...
            depth_range_samples, interval, cur_depth_min, cur_depth_max = get_depth_range_samples(cur_depth=cur_depth, 
                                                        ...
                                                        dtype=right.dtype,
                                                        device=right.device,
                                                        shape=[right.shape[0], right.shape[2], right.shape[3]],
                                                        min_depth=min_depth,
                                                        max_depth=max_depth,
            ...
            depth_rangel_samples_cv = F.interpolate((depth_range_samples//stage_scale).unsqueeze(1),
                        [self.ndisps[stage_idx]//int(stage_scale), right.size()[2]//int(stage_scale), 
                        right.size()[3]//int(stage_scale)],
                        mode='trilinear',
                        align_corners=Align_Corners_Range).squeeze(1)

            refimg_fea = []            
            for view in range(num_ref):
                refimg_fea.append(refimg_msfea[view]["stage{}".format(stage_idx + 1)])

            targetimg_fea = targetimg_msfea["stage{}".format(stage_idx + 1)]                                                 
            # matching
