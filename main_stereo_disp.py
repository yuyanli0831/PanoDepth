...
def train(target, render, depth, mask, batch_idx):
    ...
    disparity = S360.derivatives.dtheta_vertical(sgrid, depth.unsqueeze(1), baseline).squeeze(1)
    depth_loss = stereo_psmnet_loss(outputs, disparity, mask, dlossw=[0.5, 2.0])
    #loss = disp_loss
    #--------------------------------------------------
