...
def pts2sph_with_weight(pts, colors, weight, n):
    ...
    iy[iy>nPhi] = nPhi
    sphimage = np.zeros((nPhi*nTheta, 3), dtype = np.float32)
    ...
    return sphimage, sphweight

def transform_image_coor2_world(image, K, rot_mat):
    ...
    transformed_color = np.zeros((row*col, 3), dtype = np.float32)
    for ch in range(channel):
        transformed_color[:, ch] = image[:, :, ch].flatten(order='F')        
    ...
    return new_image_pos, transformed_color, weight    
