...
from network_sknet import SKNet360
...
parser.add_argument('--visualize_path', default='./visualize_monocular_refinement',
                    help='save checkpoint path')                    
...
first_network = ResNet360(norm_type='batchnorm', activation='relu', aspp=False)
second_network = ResNet360(in_channels=4, norm_type='batchnorm', activation='relu', aspp=False)
...
def train(rgb, depth, mask, batch_idx):
    ...
    optimizer.zero_grad()
    # Loss -------------------------------------------- 
    coarse_output = torch.abs(first_network(rgb))
    input_second = torch.cat([rgb, coarse_output], 1)
    ...
    depth_loss_coarse = L.direct.calculate_berhu_loss(coarse_output, depth,                
               mask=mask, weights=attention_weights)
    depth_loss_refine = L.direct.calculate_berhu_loss(outputs, depth,                
               mask=mask, weights=attention_weights)
    ...
    loss = depth_loss_coarse + depth_loss_refine + 0.1*depth_smoothness_loss        
    #--------------------------------------------------
    rgb = rgb[:,:3,:,:].detach().cpu().numpy()
    depth = depth.detach().cpu().numpy()
    depth_coarse = coarse_output.detach().cpu().numpy()
    depth_coarse[depth_coarse>8] = 0
    ...
    if batch_idx % visualize_interval == 0 and batch_idx > 0:
            ...
            depth_img = depth[0, 0, :, :]
            depth_coarse_img = depth_coarse[0, 0, :, :]
            ...
            plot.imsave('{}/depth_coarse_{}.png'.format(result_view_dir, batch_idx), depth_coarse_img, cmap="jet")
