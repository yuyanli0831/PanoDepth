...
parser.add_argument('--visualize_path', default='./visualize_monocular',
                    help='save checkpoint path')                    
...
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30, 70], gamma=0.5)
#---------------------
...
def train(rgb, depth, mask, batch_idx):
    ...
    optimizer.zero_grad()
    # Loss -------------------------------------------- 
    outputs = torch.abs(first_network(rgb))
